...
_LEGAL_FORMS = [
    ...
    'АКЦИОНЕРНОЕ КОММЕРЧЕСКОЕ', 'АКЦИОНЕРНОЕ ОБЩЕСТВО',
    'ПРОФЕССИОНАЛЬНАЯ КОЛЛЕКТОРСКАЯ ОРГАНИЗАЦИЯ', 'МИКРОКРЕДИТНАЯ КОМПАНИЯ',
    'МИКРОФИНАНСОВАЯ КОМПАНИЯ', 'ООО', 'ОАО', 'ЗАО', 'ПАО МФК', 'ПАО', 'АО', 'ПКО', 'МКК', 'МФК', 'НКО', 'КБ',
]


def _normalize_creditor_name(s):
    """Кредиторы в CBR и "Контрагент" в FinancialData — одно и то же юрлицо, но с
    разным оформлением (кавычки/ёлочки, организационно-правовая форма, "Банк" в
    конце, скобки-уточнения) — приводим к сравнимому виду для авто-сопоставления
    по выручке (см. auto_match_creditor_projects)."""
    if not s:
        return ''
    s = s.upper().replace('«', '"').replace('»', '"').replace('\xa0', ' ')
    s = re.sub(r'["\'.,]', '', s)
    s = re.sub(r'\((.*?)\)', ' ', s)
    for legal in _LEGAL_FORMS:
        s = re.sub(r'\b' + re.escape(legal) + r'\b', ' ', s)
    s = re.sub(r'\bБАНК\b', ' ', s)
    return re.sub(r'\s+', ' ', s).strip()


def auto_match_creditor_projects():
    """Автоматически сопоставляет "Текущий кредитор" (CBR) с "Проект" (П&Л) по
    совпадению нормализованного названия юрлица с "Контрагент" в выручке
    FinancialData (DCA/DP-строки) — перебирает проекты по убыванию выручки на
    это название, пока не найдёт тот, что реально есть в справочнике
    public.projects (в "Проект" из FinancialData попадаются варианты написания
    вроде "(DCA) FORT" при каноническом "(DCA) Fort" — такие пропускаются, а не
    сохраняются как есть, иначе выбор в справочнике не подсветится). Только
    ТОЧНОЕ совпадение нормализованных названий кредитора/контрагента (без
    нечёткого поиска по подстроке — на коротких токенах даёт ложные
    срабатывания). Не трогает кредиторов, у которых уже стоит любое
    сопоставление (в т.ч. явно очищенное вручную). Возвращает число вновь
    сопоставленных."""
    import pl_report as pr

    canonical_projects = {}
    for _tip, names in pr.get_projects_with_type():
        for n in names:
            canonical_projects.setdefault(n.strip().upper(), n)

    creditors = [r['creditor'] for r in query(
        "SELECT DISTINCT creditor FROM cbr.monthly WHERE creditor IS NOT NULL"
    )]
    already_mapped = {r['creditor'] for r in query('SELECT creditor FROM cbr.creditor_project_mapping')}
    fd_rows = query(
        '''SELECT "Проект" AS project, "Контрагент" AS contragent, SUM("Сумма") AS total
           FROM public."FinancialData"
           WHERE "Строка отчета" IN ('Выручка DP (цессия)', 'Выручка DCA (агентская)')
             AND "Контрагент" IS NOT NULL
           GROUP BY 1, 2'''
    )
    by_norm = defaultdict(lambda: defaultdict(float))
    for r in fd_rows:
        by_norm[_normalize_creditor_name(r['contragent'])][r['project']] += float(r['total'] or 0)

    matched = 0
    for creditor in creditors:
        if creditor in already_mapped:
            continue
        key = _normalize_creditor_name(creditor)
        if not key or key not in by_norm:
            continue
        candidates = sorted(by_norm[key].items(), key=lambda kv: -kv[1])
        resolved = next(
            (canonical_projects[p.strip().upper()] for p, _ in candidates if p and p.strip().upper() in canonical_projects),
            None
        )
        if not resolved:
            continue
        set_creditor_project(creditor, resolved)
        matched += 1
    return matched
