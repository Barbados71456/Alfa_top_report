...
SVOD_FACT_OP_LINES = [l[0] for l in SVOD_FACT_LINES]  # входят в Операционную прибыль

SVOD_FACT_BELOW = [
    ('05. Финансы (+/-)', 'Финансы'),
    ('06. Инвестиции (+/-)', 'Инвестиции'),
]
SVOD_FACT_BELOW_LINES = [l[0] for l in SVOD_FACT_BELOW]  # входят в Прибыль Компании
...
# Для плана (Budget2026) нет колонки СтатьяСвод - группируем по СтатьяУровень1/0
PLAN_BUCKETS = OrderedDict([
    ('revenue', ("Поступления по ОД (план)", "\"СтатьяУровень1\" = 'Поступления по ОД'")),
    ('expense', ("Отток по ОД (план)", "\"СтатьяУровень1\" = 'Отток по ОД'")),
])
PLAN_BELOW_BUCKETS = OrderedDict([
    ('finance', ("Финансы (план)", "trim(\"СтатьяУровень1\") = 'Финансы'")),
    ('invest', ("Инвестиции (план)", "trim(\"СтатьяУровень1\") = 'Результат по  ИД'")),
])
