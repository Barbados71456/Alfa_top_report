...
def wallet_ledger(canonical_name, date_from=None, date_to=None):
    ...
    период. Всегда "до распределения" — строки-дубликаты, появляющиеся при
    распределении косвенных расходов по проектам, помечены "Распределение" =
    'распределение' (и служебным "Кошелек" = 'распределение', см. schema.sql
    reporting.wallet_monthly) и не являются реальным движением денег по
    кошельку; для настоящих кошельков это не отфильтровывает ни одной строки
    (проверено в БД — все они и так 'до распределения'), но защищает от
